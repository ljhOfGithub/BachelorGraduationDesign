...
class Net(torch.nn.Module):
    ...
    def forward(self,data):#前向传播
        ...
        x = F.dropout(x, training=self.training)#第一层卷积->激活->dropout隐藏部分节点来调整参数->第二层卷积->softmax归一化得到分类
        x = self.conv2(x, edge_index, edge_weight)
        return F.log_softmax(x, dim=1)#softmax归一化，不是分类
...
optimizer = torch.optim.Adam([
    ...
    dict(params=model.conv2.parameters(), weight_decay=0)#设置两层优化器，自动更新参数至最优，不用设置初始值
...
def train():
    ...
    optimizer.step()#使用反向传播算法使得损失函数最小化
...
def test():
    ...
    for _, mask in data('train_mask', 'val_mask', 'test_mask'):
        pred = logits[mask].max(1)[1]#直接用取最大值方式分类，没有用sigmoid和softmax而是用pyg库中默认的ReLU激活函数
